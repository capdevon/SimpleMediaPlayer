...
public class MJPEGGenerator {
     ...
     * match input images dimension, but it is better to resize outside of java
     * and input files which are already resized.
     * @param outputHeght @see outputWidth 0
     * @param fps - Target fps. In general should match the input fps. Not really
     * required though. This info is encoded into mjpeg but the playback is
     * dependent on the value provided later on.
     * @param startNumber - order number of first and last frame. All inclusive. 0
     * and 273 mean there is file prefix000.extName to prefix273extName
     * Important. Convention assumes that the padding is consistent with the
     * number of digits of endNumber. It forces the file names to be as follows
     * (with files from 0 to 273): prefix000.extName to prefix273extName. If the
     * frames were for example 0 to 12 the files would be: prefix00.extName to
     * prefix12extName
     ...
     */
    public static void encodeImages(File outputFile, int outputWidth, int outputHeght, int fps, 
